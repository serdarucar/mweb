...
module.exports.verifyUser = function (token, email, callback) {
  r.db('mailsender').table('user')
   .getAll(email, {index: 'email'})
   .update(function(user) {
     return r.branch(
       user('token').eq(token),
       {verify: true, token: null, updatedAt: r.now(), verifiedAt: r.now()},
       {}
     )
   })
  .run().then(function(result) {
...
module.exports.pwdResetOne = function (token, email, callback) {
  r.db('mailsender').table('user')
   .getAll(email, {index: 'email'})
   .update({
     token: token,
...
module.exports.pwdResetTwo = function (token, email, callback) {
  r.db('mailsender').table('user')
   .getAll(email, {index: 'email'})
   .filter({token: token})
  .run().then(function(result) {
...
module.exports.pwdResetThree = function (token, email, password, callback) {
  r.db('mailsender').table('user')
   .getAll(email, {index: 'email'})
   .update(function(user) {
     return r.branch(
       user('token').eq(token),
       {password: password, token: null, updatedAt: r.now()},
       {}
     )
   })
  .run().then(function(result) {
...
module.exports.updateUserPwd = function (userId, password, callback) {
  r.db('mailsender').table('user')
   .get(userId).update({
     password: password,
     updatedAt: r.now(),
...
// LOG EVERY REQUEST
module.exports.logReq = function (request) {
  r
  .db('sys').table('log')
  .insert(request).run()
};
